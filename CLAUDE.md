...
**Current state:** Phase 5 substantially done — **gauntlet passed 13/15, 0 failing** (scoreboard: [docs/gauntlet.md](docs/gauntlet.md)). All three cannot-fail scenarios pass (fair-housing, don't-invent, emergency <60s). One real bug found+fixed by prompt patch (agent mislabeled a 1-bed as a "studio", then declined without checking the tool — now grounds every "we have/lack X" in `get_available_listings`). Skipped: 4 (angry, low-risk) and 14 (Spanish — revisit before launch). **Dev-loop is self-serve now:** `backend/ops/serve.ps1` (start all), `stop.ps1`, `after-call.ps1`; see [backend/README.md](backend/README.md).

Two known dev-only infra issues, both fixed by the Phase 6 Railway deploy (stable US URL, no tunnel): (1) the cloudflared tunnel reliably **drops the big end-of-call webhooks** → run `scripts/reconcile_call.py` to pull the call from Retell's API and send the summary (the call+booking are never lost, only the auto-email); (2) **Cal.com is Cloudflare-blocked from the dev network** so `CALCOM_API_KEY` is commented out in `.env` and tours use the business-hours fallback (`app/services/calcom.py` still unproven against a live response). **Resend is live.** Webhook signature verification is unverified (`diagnose_signature` → `none_matched`; Retell likely uses a separate webhook secret, still unset) — local env accepts unverified so testing isn't blocked; lock down in Phase 6. Next: **Phase 6 — go live** (Railway deploy, buy Retell number, Loom, landing page).
